          ...
          {slides.map((item, index) => (
            <Carousel.Item key={index}>
              <Slide imgAlt={t(item.imgAlt)} imgSrc={item.imgSrc}
                ...
                linkClass="Btn-S Btn-L-X-W font-weight_700"
      ...
      <section id="Overlays" className="B-B Flex A-I-C J-C-C">
        ...
        <div className="GridWrapper Grid">
          {overlays.map((item, index) => (
            <Overlay key={index} id={item.id} imageName={item.imageName}
              alt={t(item.alt)}
              imageType={index % 2 ? 'main.vertical' : 'main.horizontal'} type="link"
              linkClassName={`Btn-S ${(width < 481
                ? (t(item.buttonText).length > 12
                ? 'Btn-Sm-X-W'
                  : 'Btn-Sm')
                : 'Btn-Sm-X-W')}`}
