...
export default function ProductsList() {
  ...
  return (
    <LoadingOverlay
      ...
      <Head title={`${category} ${t('productsList.seo.title')}`} description={t('productsList.seo.description')}
        cardTitle={`${category} ${t('productsList.seo.title')}`} cardDescription={t('productsList.seo.cardDescription')}
        imgUrl={cardUrl} imgUrlSecure={cardUrl}
              ...
              return (
                <li key={product.id} className="Flex J-C-C A-I-C F-F-C-N">
                  <Link to={location.pathname + '/' + product.id}>
                    <Card className="Grid Image-List-C">
                      <Overlay src={product.imgSrc} alt={product.imgDescription}>
                        <h2>{product.name}</h2>
                      </Overlay>
                      <div className="It-Inf Flex A-I-C J-C-S-B F-F-C-N T-C F-H">
                        ...
                        <div className="Prod-Pr Grid F-W">
                          <p className="Flex A-I-C J-C-C font-weight_600 h4-size error-300 F-W">
                            {isDiscounted
                              ? t('productsList.priceDiscounted', { price: product.price * (100 - product.discount) / 100 })
                              : t('productsList.price', { price: product.price })}
                          </p>
                          {isDiscounted ? (
                            <p
                              className="Flex A-I-C J-C-C font-weight_200 helper Crossed F-W">
