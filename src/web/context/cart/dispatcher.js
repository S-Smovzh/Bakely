...
export const addProductToCart = (product, state) => {
  if (typeof window !== 'undefined') {
    ...
    if (updatedItem) {
      ...
      updatedItem.total = updatedItem.price * (100 - updatedItem.discount) / 100 *
        updatedItem.selectedOption.split(' ')[0] * updatedItem.quantity;
...
export const loadProducts = (state) => {
  const loadedCart = typeof window !== 'undefined' && (localStorage.getItem(btoa('cartItems')) ?
...
export const clearCart = (state) => {
  typeof window !== 'undefined' && localStorage.removeItem(btoa('cartItems'));
  return { ...state, cart: [] };
};
