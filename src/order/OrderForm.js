...
const OrderForm = () => {
  ...
  return (
    <Modal
      className={`Modal Order-Form Flex J-C-C A-I-C 
      ${currentPage === 1 ? 'Introduction' : ''}
      ${currentPage === 2 ? 'Contact' : ''}
      ${currentPage === 3 ? 'Option' : ''}
      ${(currentPage === 4 && orderForm.delivery) ? 'Delivery' : ''}
      ${(currentPage === 4 && orderForm.selfPickUp) ? 'SelfPickUp' : ''}
      ${currentPage === 5 ? 'Success' : ''}`
      }
      onHide={() => closeModal()}
    ...
    return (
      <Animation onClick={!nextButtonDisabled} onHover={!nextButtonDisabled}
        ...
        <button
          ...
          ${(page === 5 || page === 2 || page === 4 ? 'Btn-Su ' : 'Btn-S ')} +
          ${nextButtonAnimation === 'error-shake' ? 'Error-Shake' : ''}`}
